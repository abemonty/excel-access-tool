...
public class ExcelRowInvocationHandler implements InvocationHandler {
    ...
    public static HSSFCell createCellFromMethod(HSSFRow row, Method method, int columnDelta) {
        ...
        if (excelCell == null) {
            LOGGER.warn("Impossible de créer la cellule voulue, la méthode ne possède pas l'annotation ExcelCell ("
                    + method.getName() + ")");
            return null;
        }
        ...
        if (cellTypeId != CELL_TYPE_BLANK) {
            ...
            LOGGER.debug("création de la cellule " + cellColumnId + columnDelta + "(" + cellTypeId + ", "
                    + returnType.getName() + ")");
        } else {
            LOGGER.warn("création de la cellule " + cellColumnId + columnDelta + " non effectuée, type non géré : "
    ...
    /**
     * Création du handler qui va s'occuper de gérer les méthodes du proxy pour une ligne particulière représentée par
     ...
     * <b>Il y a des problèmes de lecture d'un excel car des cases sont positionnées automatiquement par POI sur un type
     * (double par exemple) et on veut les lire sur un autre type (String).</b>
     * 
     * @param method
    ...
    protected Object doGet(Method method, Integer index) {
        ...
        int columnDelta = 0;
        if (index >= 0) {
            RepeatableExcelCell repeatableExcelCell = ExcelHandlerUtils
                    .getAnnotation(RepeatableExcelCell.class, method);
        ...
        /*
         * TODO (loic) : cette gestion des types numériques ne me plait pas, est-ce qu'un type primitif peut renvoyer
         * null? Je ne pense pas.
         */
            ...
            if (value != null) {
                ...
                if (StringUtils.isNotBlank(stringFormat)) {
                    try {
                        ...
                        LOGGER.warn(
                                "Erreur de format de la donnée de la ligne {}, colonne {} avec le format {}. La méthode concernée est {}",
